...
def register_student(roll_no, name, department, year, division, image_path, subject_ids, email=None):
    ...
    try:
        ...
        for subject_id in subject_ids:
            print("➡ inserting:", student_id, subject_id)
            cursor.execute('''
        ...
        # VERIFY
        cursor.execute("SELECT * FROM student_subjects WHERE student_id = ?", (student_id,))
...
def get_student_attendance_summary(student_id):
    """Get summary of attendance for a student across all enrolled subjects"""
    conn = get_connection()
    cursor = conn.cursor()
    ...
    SELECT
        ...
        COALESCE((
            SELECT COUNT(DISTINCT a2.date || '|' || a2.period)
            FROM attendance a2
