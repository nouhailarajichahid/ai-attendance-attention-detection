...
st.set_page_config(
    page_title="AI Student Attendance & Attention System",
    page_icon="👨‍🎓",
    layout="wide",
...
if st.sidebar.button("🧠 Attention Monitoring", key="nav_attention", use_container_width=True):
    st.session_state.page = "Attention Monitoring"

if st.sidebar.button("📋 Combined Report", key="nav_combined", use_container_width=True):
    st.session_state.page = "Combined Report"
...
if page == "Teacher Dashboard":
    ...
    st.markdown('<div class="dashboard-card"><p>View and manage student information for CS 2026 A division.</p></div>', unsafe_allow_html=True)
    ...
    try:
        ...
        if not all_students:
            ...
            if not filtered_df.empty:
                ...
                if selected_roll:
                    try:
                        student_id = int(filtered_df[filtered_df["Roll No"] == selected_roll]["ID"].values[0])
    ...
    with st.form("student_registration"):
        ...
        # Subject selection
        try:
            subjects = get_subjects()   # [(id, code, name), ...]

            # show full readable labels, but keep id mapping safe
            subject_map = {
                f"{sub[1]} - {sub[2]}": sub[0]   # "AWP - Antenna and Wave Propagation" -> 1
                for sub in subjects
            }
        ...
        if submitted:
            if not name or not roll_no or not face_image or not selected_subjects:
                ...
                try:
                    ...
                    subject_ids = [subject_map[label] for label in selected_subjects if label in subject_map]

                    if not subject_ids:
                        st.error("No valid subjects were selected. Please select at least one subject.")
                        st.stop()                    
                    ...
                    if student_id:
                        ...
                        st.write(f"Saved subject links in database: {len(saved_subjects)}")

                        if len(saved_subjects) > 0:
                            if select_all:
                                st.success(f"Enrolled in all {len(saved_subjects)} subjects")
                            else:
                                st.success(f"Enrolled in {len(saved_subjects)} subjects")
                        else:
                            st.error("Student was created, but no subjects were saved.")
                    else:
                        st.error("Failed to register student. Roll number may already exist or subjects were invalid.")
                except Exception as e:
            ...
            for idx, img_file in enumerate(image_files):
                with st.spinner(f"Processing image {idx+1} of {len(image_files)}..."):
                    try:
                        ...
                        # Initialize lists to store matched students and confidence scores
                        present_students = []
                        confidence_scores = []

                        # Load all registered students from database
...
elif page == "Attention Monitoring":
    st.title("🧠 Attention Monitoring")
    st.markdown("Real-time student attention detection using facial analysis.")

    # Import attention detector
    try:
        from utils.attention_detector import AttentionDetector, save_attention_record
        attention_available = True
    except ImportError as e:
        st.error(f"Attention module not available: {e}")
    ...
    if attention_available:
        ...
        try:
            ...
            if not all_students:
                ...
                with col1:
                    duration = st.slider(
                        "Monitoring Duration (seconds)", 10, 120, 30)
                with col2:
                    st.metric("Selected Student", selected_label.split(" - ")[1])

                st.divider()

                # Initialize detector in session state
                if "attention_detector" not in st.session_state:
                    st.session_state.attention_detector = None
                ...
                with col_start:
                    if st.button("▶ Start Monitoring", type="primary",
                        ...
                        if st.session_state.attention_detector is None:
                            st.session_state.attention_detector = AttentionDetector()
                        else:
                            st.session_state.attention_detector.reset()
                ...
                if st.session_state.monitoring_active:
                    st.info("Camera is active. Look at the camera.")

                    FRAME_WINDOW = st.image([])
                    status_col, score_col, ear_col = st.columns(3)

                    cap = cv2.VideoCapture(0)
                    detector = st.session_state.attention_detector
                    ...
                    while st.session_state.monitoring_active:
                        ...
                        # Show frame
                        rgb_frame = cv2.cvtColor(result["frame"],
                                                 cv2.COLOR_BGR2RGB)
                        ...
                        status_placeholder.markdown(
                            ...
                            f"**EAR:** `{result['ear']:.3f}` | "
                            f"**Time left:** `{int(duration-elapsed)}s`"
                        )
                    ...
                    # Session results
                    if score_list:
                        avg_score  = round(float(np.mean(score_list)), 1)
                        # Most common status
                        final_status = max(set(status_list),
                                           key=status_list.count)

                        st.success("✅ Monitoring session complete!")

                        r1, r2, r3 = st.columns(3)
                        r1.metric("Average Attention", f"{avg_score*10}%")
                        r2.metric("Final Status", final_status)
                        r3.metric("Duration", f"{int(elapsed)}s")

                        # Save to database
                        saved = save_attention_record(
                        ...
                        if len(score_list) > 1:
                            ...
                            chart_df = pd.DataFrame({
                                "Frame":   list(range(len(score_list))),
                                "Attention %": score_list
                            })
    ...
    st.title("📋 Combined Report")
    st.markdown("Attendance + Attention summary for all students.")

    try:
        from utils.attention_detector import get_attention_summary
        ...
        if not all_students:
            ...
            attention_data = get_attention_summary(date_str)
            attention_map  = {
                row["id"]: row for row in attention_data
            }
            ...
            for student in all_students:
                ...
                avg_score   = attn.get("avg_score", None)
                last_status = attn.get("last_status", "No Data")
                ...
                attendance_status = (
                    f"✅ {present_count}/{total_subjects}"
                    if total_subjects > 0 else "No Record"
                )
            ...
            with_attention = [r for r in attention_data
                              if r.get("avg_score") is not None]
            if with_attention:
                avg_class = round(
                    sum(r["avg_score"] for r in with_attention)
                    / len(with_attention), 1)
                m2.metric("Class Avg Attention", f"{avg_class}%")
            else:
                m2.metric("Class Avg Attention", "No Data")

            attentive_count = sum(
                1 for r in attention_data
                if r.get("last_status") == "Attentive"
            )
            m3.metric("Attentive Students", attentive_count)

            # Export combined Excel
            if st.button("📥 Export Combined Excel"):
                import pandas as pd
                excel_path = os.path.join(
                    "excel_exports",
                    f"Combined_Report_{date_str}.xlsx"
                )
                with pd.ExcelWriter(excel_path, mode='w') as writer:
                ...
                with open(excel_path, "rb") as f:
                    st.download_button(
                        ...
                        data=f,
                        file_name=f"Combined_Report_{date_str}.xlsx",
                        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                    )
...
st.sidebar.divider()
st.sidebar.info(
"AI Attendance & Attention Detection System\n"
